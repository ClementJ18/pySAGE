...
with a polished-steel trim - and there is one skin per Edain faction besides, each sampled from
that faction's own emblem: the dark field the emblem is drawn on sets the ground, its brightest
accent sets the trim, and every other token is a fixed function of those two. The generator that
derived them left with the Edain overlay - the emblem art it read is not in this repository any
more - so the block at the foot of this module is now maintained by hand.
...
# Derived from the Edain faction emblems (see the module docstring). Generated originally, now
# hand-maintained: `ruff format` would unpack the grouped layout, hence the fmt fence below.
# fmt: off
