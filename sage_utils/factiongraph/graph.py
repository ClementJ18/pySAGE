...
- **Plot flags** (BFME1-style build plots, reintroduced by Edain): flag objects'
  ...
  side (a caller that knows its mod's canonical flag names - a mod overlay such as
  `sage_edain` - passes them instead for precision). A base *layout* (a `.bse` name, not an ini
  object) is decomposed through a layout resolver: by default the table
  `sage_utils.sources.load_sources` attaches to the game (`game.base_layouts`, swept from the
  loaded archives); an overlay can instead wire an on-disk `bases/`-folder resolver. Without
  either, the start point records the base name and contributes no seeds.
...
# (see `game_base_layout`); a mod overlay supplies one backed by a mod checkout's `bases/`.
...
def start_points(
    ...
    (a mod's canonical plots, supplied by its overlay), else every flag `plot_flags` discovers -
...
def build_faction_graph(
    ...
    table the source loader attached to the game; a mod overlay wires its own `bases/`-folder
    resolver here)."""
