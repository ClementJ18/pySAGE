...
class PlayerState:
    ...
    power_points: int = 0
    # The sciences this player holds, **by id rather than by name**: a spellbook power bought
    # from the spell store, the faction's own intrinsic science, and the four view sciences
    # every seat carries. This is the direct answer to "have I already bought that power",
    # which nothing else observable gives - a science is not an upgrade and lands in none of
    # the upgrade masks.
    #
    # Ids and not names because naming them needs an ini load and this package's backends do
    # not have one: `TheUpgradeCenter` hands out upgrade names in-process, and the equivalent
    # walk of `TheScienceStore` does not reach a name (see `sage_live.utils.naming`). The id is
    # the same number `orders.purchase_power` sends and the same one
    # `sage_live.utils.resolve.Resolver.science` returns, so a consumer that can name a science
    # at all can test membership here: `names.science("SCIENCE_RebuildMen") in player.sciences`.
    ...
    def command_points_free(self) -> int:
        """Army capacity still available - real headroom, in command points.

        **The cap is enforced, and this used to under-report it by every bonus the player had
        bought.** The pair's cap half was read from the *base* field alone, which is a flat 500
        for every seat and never moves, so a player who had built `CPObject`s looked permanently
        over their ceiling: usage of 1262 against a reported 500. That made this clamp to zero
        while real capacity remained, and it made the opposite error too - reading a comfortable
        28 when a battalion costs 60 and the recruit will be refused.

        `MemoryBackend` now sums the base with the bonus at `+0x6C`, which is the figure the
        engine checks: measured live across a whole match's seats, usage tracked `base + bonus`
        and never crossed it. See the layout note on `player_command_points_bonus`.

        Still worth pairing with the direct signal rather than trusting the arithmetic alone - a
        recruit that does not queue is what `Session.confirm_queued` reports, and it needs no
        offset to be right.
        """
...
class GameObject:
    ...
    # The *other* horde link: an `ObjectID` the engine keeps beside the object's own, naming
    # whatever produced it. Unlike `parent_id` it survives the object leaving what it names, and
    # the engine falls back to it when `parent_id` is empty - so a loose unit is still resolved
    # back to the battalion it came out of. `None` when nothing produced this object.
    #
    # A member whose `parent_id` is None while `producer_id` still names a live horde is exactly
    # the half-formed state in `sage_patch/docs/horde-formation-orphans.md`.
    producer_id: int | None = None
    # The engine's `ObjectStatus` bits: what the game asks about an object before acting on it.
    # `HORDE_MEMBER` is what makes a unit part of a battalion at all, and `IS_LEAVING_FACTORY`
    # marks one still coming out of the building that made it - so "this battalion has not
    # finished forming" is readable here and in no other field. Empty for an object in no
    ...
    def is_rubble(self) -> bool:
        """Whether this structure has been destroyed and is standing as wreckage.

        **A destroyed building is not always a building that has gone.** A camp or castle keep
        stays in the object table at zero health wearing the engine's `RUBBLE` condition, so it
        is owned, has a body, is not under construction, and reads through `is_damaged` as the
        most damaged thing on the map. Measured across two live runs: the heal picked a
        `GondorCampKeep` and a `GondorCastleBaseKeep` "at 0%" and spent its recharge on wreckage.

        Health cannot answer this either, in the same way `under_construction` cannot: zero is
        also what a structure reads on the frame it finishes dying, and a low number is just a
...
class Observation:
    ...
    Nothing here is fog-filtered *as produced*. The engine knows what each player can see, but
    that filter belongs on the game side and must be applied deliberately - training a policy on
    information a human never had should be a decision, not an accident. `under_fog` is the
    decision; `shroud` is what makes it possible.
    """
    ...
    fogged: bool = False
    # The engine's own visibility grid, when the backend could read one. Present on a whole-map
    # observation and carried through `under_fog`, so a policy can still ask what it can see
    # after the objects it cannot see are gone - which is what scouting decisions need.
    #
    # **Not carried over the wire.** `encode_observation` has no field for it, so a decoded
    # observation has `shroud=None` and `fogged` alone records whether the filter ran.
    ...
    def orderable(self, index: int) -> tuple[GameObject, ...]:
        ...
        **Horde members are excluded, and this is not a nicety.** A battalion appears as its
        ...
        contains. The name-based test in `sage_live.utils.statics` reaches the same answer from ini
    ...
    def under_fog(self, viewer: int | None = None, keep_own: bool = True) -> Observation:
        ...
        **This is visibility, not knowledge** - the other half of the pair `without_godsight`
        opens. That one leaves every object in place and strips fields a player could not read;
        this one removes the objects themselves, which is what stops a policy from countering an
        army it has never scouted.

        Objects are kept when the viewer's shroud level at their position is `CLEAR`. `keep_own`
        additionally keeps everything the viewer owns, which costs nothing in honesty - you can
        always see your own army - and covers the handful of own objects whose position does not
        land on a revealed cell, measured live at 1 of 70 (a garrisoned unit reports its holder's
        position, and a unit right on the grid edge rounds outward).

        **Returns the snapshot unchanged, with `fogged` still False, when there is no grid to
        filter by, or when the match runs with fog switched off.** Both are honest: an absent
        grid means the backend could not read one, and silently hiding the whole map would look
        exactly like a working filter on an empty battlefield. Check `fogged` on the result if it
        matters whether the filter actually ran.

        **Position only, and centre only.** The engine samples an object's bounding circle -
        centre plus four points at its geometry radius - so a structure wider than the 40-unit
        cell becomes visible to the engine slightly before its centre cell clears. This tests the
        centre, so it hides such a structure for a moment longer than the game does. The
        direction is the safe one, and closing it needs a geometry radius the object model does
        not yet read.

        **No memory of what was seen before.** A building you scouted and walked away from
        disappears here, where a real player would still see it drawn. That is a real difference
        from the game, and it is a limit of the engine state this reads rather than a choice:
        the per-cell grid has no explored-but-not-seen state at all, and the per-object flag that
        does (`m_everSeenByPlayer`) is not located. A consumer that wants a scouting memory
        should accumulate one across frames - it is the consumer's memory, and making it explicit
        is better than pretending the engine supplied it. See `sage_live.backends.shroud`.
        """
