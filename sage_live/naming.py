...
`sage_live.orders` takes resolved integers, and something has to turn `"MordorFighterHorde"`
...
- **an ini load**, via `sage_live.resolve.Resolver`. Works offline and covers all four id
...
class LiveNames:
    ...
    def thing(self, name: str) -> int:
        ...
        **Corroborated against the ini table, and the divergence is confined to the tail.**
        ...
        build the thing it names. `sage_live.resolve.Resolver` remains the path to prefer when
    ...
    def _unwalked(space: str, store: str) -> NoNameLookup:
        return NoNameLookup(
            ...
            f"walked). Attach a `sage_live.resolve.Resolver` for {space} names, or pass an id."
        )
