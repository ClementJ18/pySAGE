...
>
> Twenty-three of the registered patches — **`battle-school`**, **`campaign-army-verbs`**,
...
> **`spellbook-commandset-refresh`**, **`standalone-launcher`**, **`unit-plate-option`** and
> **`wotr-battle-observers`** — are
...
- **`spellbook-commandset-refresh`** checks the persistent left spellbook bar's resolved
  ...
  separate from the spellstore window and leaves CommandSetUpgrade's stock UI tail unchanged.
  No INI change. **Its core function is runtime-verified in game**, both for an ordinary
  CommandSet switch and with `commandset-button-upgrade`'s `CommandButtons`; it remains
  experimental pending lifecycle, long-session and multiplayer checks. See
