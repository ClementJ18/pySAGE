...
| [`docs/module-types.json`](docs/module-types.json) | what a field's type alone cannot say: the members behind every enum and flag type, the lookup lists, and the keywords (with types, where the engine uses a field table) of every nested sub-block. Same generator. |
| [`scripts/build_wiki.py`](scripts/build_wiki.py) | renders those two files into a browsable static site - a page per module, plus the type and enum pages - under `build/wiki/`. Each module page takes values in its field table, checks them against the field's type and writes them into a copyable INI block. |
...
python scripts/module_defaults.py game.dat \
    ...
    --enums docs/module-types.json      # needs your own game.dat
...
field table a nested block hands to the INI reader. The site is a build artefact and is
not committed; the JSON it reads is.
