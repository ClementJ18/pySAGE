...
pip-installed `sage_ini` can opt into the agent workflow. The skill is static — it drives the
`sage-ini` CLI live rather than embedding generated data — so installing is a plain copy of the
...
def _packaged_skill() -> Traversable:
    return resources.files("sage_ini").joinpath("skill_assets", SKILL_NAME)
