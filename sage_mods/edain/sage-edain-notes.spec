...
# Build the standalone Edain Patch Notes window:
#   pyinstaller sage_mods/edain/sage-edain-notes.spec
# The result is one single-file binary in dist/:
#   - Edain Patch Notes(.exe) - the window that turns the release spreadsheet's CSV export into
#     the English and German BBcode posts, windowed-subsystem, for whoever writes the notes to
#     run without Python.
# It bundles PyQt6, so building needs the [edain-ui] extra installed. Kept apart from
# sage-edain.spec (the CLI + the Edain Linter) so the notes window can be rebuilt and handed
...
UI = os.path.join(ROOT, 'sage_mods', 'edain', 'patch_notes', 'ui')
